<template>
  <div class="page">
    <van-nav-bar class="ops-nav" title="其它入库单" left-arrow @click-left="router.back()">
      <template #right>
    ...
    <div class="list-shell">
      <van-list v-model:loading="loading" :finished="finished" finished-text="没有更多了" @load="loadMore">
        <div
          ...
          :key="row.id"
          class="order-card"
          @click="router.push(`/warehouse/other-inbounds/${row.id}`)"
        >
    ...
    <van-popup v-model:show="showCreate" position="bottom" round teleport="body" class="sheet-popup" style="height: 85%">
      <div class="sheet create-doc">
        <div class="sheet-title">新建其它入库</div>
        <van-field
        ...
        <div v-for="(it, idx) in form.items" :key="idx" class="card">
          ...
          <van-field v-model="it.cost" type="number" label="成本" />
...
function emptyItem() {
  return { invSkuId: 0, skuCode: '', skuKw: '', qty: '1', cost: '0' }
}
...
async function create() {
  ...
  creating.value = true
  try {
    const row = await whApi.createOtherIn({
    ...
    router.push(`/warehouse/other-inbounds/${row.id}`)
  } catch (e: any) {
...
async function loadMore() {
  loading.value = true
  try {
    const res = await whApi.listOtherIn({ page: page.value, pageSize: 20 })
    const rows = res.list || []
